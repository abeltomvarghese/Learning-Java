...
public class ConsoleUI {
    ...
    private static void addConnection() {
        Node startNode = getNode("Enter index of start node ");
        Node targetNode = getNode("Enter index of target node ");

        //NOTE TO STUDENTS - IT IS GENERALLY BEST TO AVOID USING THE
        //instanceof OPERATOR, UNLESS YOU REALLY HAVE TO!
        if (startNode instanceof DecisionNode) {
            System.out.print("Type Y to add a connection when decision is Yes, or N for No>");
            String contype = scanner.nextLine();
            ((DecisionNode) startNode).setNext(targetNode, contype.startsWith("Y"));
        } else if (startNode instanceof ActionNode) {
            ((ActionNode) startNode).setNext(targetNode);
        }

    }
    
    private static Node getNode(String prompt) {
        int nbrNodes = flowChart.getNbrNodes();
        int nodeIdx;
        do {
